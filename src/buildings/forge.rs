...
//! Shard metadata and recipes live in the shard catalog (`lib-inventory/src/shard_catalog.rs`);
//! the Forge only reads them from the [`Almanach`].

use std::time::Duration;

use lib_ui::prelude::{CostIndicator, TooltipOf};
...
impl Plugin for ForgePlugin {
    fn build(&self, app: &mut App) {
        ...
        app
            ...
            .add_observer(BuilderForge::on_add)
            .add_observer(on_start_forge)
            .add_observer(on_cancel_forge)
            .add_observer(ForgeInfoPanel::on_building_info_panel_enabled)
            .add_observer(ForgeInfoPanel::on_rebuild_forge_ui)
            .add_observer(ForgeShardButton::on_add)
            .add_observer(on_forge_job_insert)
            .add_observer(on_forge_job_remove)
            .register_db_loader::<BuilderForge>(MapLoadingStage::SpawnMapElements)
            .register_db_saver(BuilderForge::on_game_save)
...
impl ForgeInfoPanel {
    ...
    fn on_rebuild_forge_ui(
        ...
        if let Some(job) = maybe_job {
            ...
            )).observe(ForgeCancelButton::on_click).id();
