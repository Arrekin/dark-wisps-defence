...
// Mesh padding factor. Mirrors `WispWaterMaterial::QUAD_SCALE` in wisps_internal/src/materials.rs:
...
fn dwd_wisp_water(uv: vec2<f32>, time: f32, look: WispWaterLook, effects_mask: u32) -> vec4<f32> {
    ...
    let deform = DEFORM_ASYMP * look.vigor / (look.vigor + DEFORM_ASYMP - 1.0);

    // Travel direction (falls back to a fixed axis while at rest, where deform ≈ 0).
    let heading_raw = vec2<f32>(look.heading_x, look.heading_y);
    ...
    let stroke_rate = STROKE_RATE_REST + STROKE_RATE_SWING * look.vigor;
    let stroke = look.stroke_anchor_phase + (time - look.anchor_time) * stroke_rate;
    let beat = sin(stroke + look.seed);
    let lunge = deform * (STRETCH_BASE + STRETCH_BEAT * beat);
    let lurch = deform * LURCH * beat;
    let sway = deform * SWAY * sin(stroke * 0.5 + look.seed * 3.0);
    ...
    let u = (uv - vec2<f32>(0.5)) * QUAD_SCALE;
    ...
    let surf_rate = SURF_RATE_REST + SURF_RATE_SWING * look.vigor;
    let surf_t = look.surf_anchor_phase + (time - look.anchor_time) * surf_rate;
    let wob = look.wobble * (1.0 + CHURN_AMP_GAIN * deform);
    let direction_to_edge = q / max(length(q), 1e-4);
    let trailing = max(0.0, -dot(direction_to_edge, h)); // 1 on the trailing side
    var edge = 0.9
        + wob * sin(ang * 3.0 + surf_t + look.seed)
        + wob * 0.55 * sin(ang * 5.0 - surf_t * 1.3 + look.seed * 2.0)
    ...
    col += vec3<f32>(-0.02, 0.03, 0.04) * look.tint; // per-instance hue shift
    ...
    if ((effects_mask & BRITTLE) != 0u) {
        color = brittle(color, q, body, look.seed);
    }
    return color;
}
