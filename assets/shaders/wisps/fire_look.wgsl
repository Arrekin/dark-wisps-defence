...
// Mesh padding factor. Mirrors `WispFireMaterial::QUAD_SCALE` in wisps_internal/src/materials.rs:
...
fn fire_corona(p: vec2<f32>, t: f32, seed: f32, wind: vec2<f32>) -> f32 {
    ...
    for (var k = 0; k < NUM_LICKS; k = k + 1) {
        ...
        // Rotate side-facing licks toward the wake to form a narrow trail. The 2D cross product
        // varies continuously with angle, peaks at the sides, and reaches zero at the front and
        // back, preventing direction changes from snapping.
...
// it stays concentric to the core. Gold contrasts the fire body. Static, seeded per instance.
// ─────────────────────────────────────────────────────────────────────────────────
...
fn brittle(color: vec4<f32>, c: vec2<f32>, seed: f32) -> vec4<f32> {
...
fn dwd_wisp_fire(uv: vec2<f32>, time: f32, look: WispFireLook, effects_mask: u32) -> vec4<f32> {
    ...
    let pulse = 0.97 + 0.03 * sin(t * PULSE_SPEED + look.seed);
    ...
    let wind = -vec2<f32>(look.heading_x, look.heading_y) * min(look.vigor, 1.5);
    let flame = fire_corona(p, t, look.seed, wind);
