...
impl BuilderBrittleEffect {
    pub fn new(target_entity: Entity, damage_multiplier: f32) -> Self {
        Self { target_entity, damage_multiplier, expires_at: None, save_data: None }
    }
    ...
    fn new_for_saving(target_entity: Entity, damage_multiplier: f32, save_data: BrittleEffectSaveData) -> Self {
        Self { target_entity, damage_multiplier, expires_at: None, save_data: Some(save_data) }
    }
}
impl Saveable for BuilderBrittleEffect {
    fn save(self, tx: &rusqlite::Transaction) -> rusqlite::Result<()> {
        ...
        tx.execute(
            "INSERT OR REPLACE INTO brittle_effects (id, target_id, damage_multiplier, expires_at) VALUES (?1, ?2, ?3, ?4)",
            rusqlite::params![entity_index, target_index, self.damage_multiplier, self.expires_at.as_ref().map(|e| e.0)],
        )?;
        Ok(())
    }
}
impl Loadable for BuilderBrittleEffect {
    fn load(ctx: &mut LoadContext) -> rusqlite::Result<LoadResult> {
        let mut stmt = ctx.conn.prepare(
            "SELECT id, target_id, damage_multiplier, expires_at FROM brittle_effects LIMIT ?1 OFFSET ?2"
        )?;
        ...
        while let Some(row) = rows.next()? {
            ...
            let damage_multiplier: f32 = row.get(2)?;
            ...
            let mut builder = BuilderBrittleEffect::new_for_saving(new_target, damage_multiplier, save_data);
...
impl BuilderBrittleEffect {
    fn on_game_save(
        ...
        brittle_effects: Query<(Entity, &EffectTarget, &ModifierContributions, Option<&ExpiresAt>), With<BrittleEffect>>,
    ) {
        if brittle_effects.is_empty() { return; }
        let batch = brittle_effects.iter().map(|(entity, effect_target, contributions, expires_at)| {
            let damage_multiplier = contributions.0
                .get(&ModifierType::IncomingDamageMultiplier)
                .copied()
                .unwrap_or(1.0);
            let save_data = BrittleEffectSaveData { entity };
            let mut builder = BuilderBrittleEffect::new_for_saving(effect_target.0, damage_multiplier, save_data);
