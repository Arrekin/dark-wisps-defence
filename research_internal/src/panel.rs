...
#[derive(Event)]
pub(crate) struct RebuildResearchPanel;
...
impl ResearchPanelRoot {
    fn on_add_build_panel_scene(trigger: On<Add, ResearchPanelRoot>, mut commands: Commands) {
...
pub(crate) struct ResearchPanelContent;
...
pub(crate) fn register(app: &mut App) {
    app
        ...
        .add_systems(OnEnter(UiInteraction::ResearchPanel), show_research_panel)
        .add_systems(OnExit(UiInteraction::ResearchPanel), hide_research_panel)
        .add_systems(Update, update_card_progress.run_if(in_state(UiInteraction::ResearchPanel)))
        .add_observer(ResearchPanelRoot::on_add_build_panel_scene)
        .add_observer(rebuild_panel)
        .add_observer(on_insert_active_research_request_rebuild)
        .add_observer(on_remove_active_research_request_rebuild)
        .add_observer(on_research_completed_request_rebuild)
        .add_observer(on_insert_obsolete_request_rebuild)
        .add_observer(on_remove_obsolete_request_rebuild)
        .add_observer(on_research_instantiated_request_rebuild)
        ;
}

fn show_research_panel(
    mut commands: Commands,
...
fn hide_research_panel(root: Single<&mut Visibility, With<ResearchPanelRoot>>) {
...
fn on_insert_active_research_request_rebuild(_trigger: On<Insert, ActiveResearch>, mut commands: Commands) {
    commands.trigger(RebuildResearchPanel);
}
fn on_remove_active_research_request_rebuild(_trigger: On<Remove, ActiveResearch>, mut commands: Commands) {
    commands.trigger(RebuildResearchPanel);
}
fn on_research_completed_request_rebuild(_trigger: On<ResearchCompleted>, mut commands: Commands) {
    commands.trigger(RebuildResearchPanel);
}
fn on_insert_obsolete_request_rebuild(_trigger: On<Insert, Obsolete>, mut commands: Commands) {
    commands.trigger(RebuildResearchPanel);
}
// Future-proofs revocation: a research becoming non-obsolete again should reappear.
fn on_remove_obsolete_request_rebuild(_trigger: On<Remove, Obsolete>, mut commands: Commands) {
    commands.trigger(RebuildResearchPanel);
}
fn on_research_instantiated_request_rebuild(_trigger: On<ResearchInstantiated>, mut commands: Commands) {
    commands.trigger(RebuildResearchPanel);
}
    ...
    bsn! {
        ...
        Children [
            ...
            (
                ...
                on(ResearchStartStopButton::on_click_toggle_research)
                Children [
